'use client'

import { useState, useEffect, useRef } from 'react'
import { Trash2, Bot, FileDown, FileText } from 'lucide-react'
import type { Activity } from '@prisma/client'

// ─── Costanti ─────────────────────────────────────────────────────────────────

const ACTIVITY_LABELS: Record<string, string> = {
  SPIEGAZIONE: 'Spiegazione',
  ESERCIZI: 'Esercizi',
  VERIFICA: 'Verifica',
  SCHEDA: 'Scheda',
  DIAPOSITIVE: 'Diapositive',
  COMPITO: 'Compito',
}

// ─── Utility: parsing HTML → sezioni con ID ───────────────────────────────────

function parseAndInjectIds(html: string): {
  processedHtml: string
  sections: Array<{ id: string; label: string }>
} {
  const sections: Array<{ id: string; label: string }> = []
  let counter = 0
  const processedHtml = html.replace(
    /<h2([^>]*)>([\s\S]*?)<\/h2>/gi,
    (_, attrs: string, inner: string) => {
      const id = `toc-${counter++}`
      const label = inner.replace(/<[^>]+>/g, '').trim()
      sections.push({ id, label })
      return `<h2${attrs} id="${id}">${inner}</h2>`
    }
  )
  return { processedHtml, sections }
}

// ─── Utility: export PDF via stampa browser ───────────────────────────────────

function exportPDF(title: string, content: string) {
  const win = window.open('', '_blank')
  if (!win) {
    alert('Abilita i popup per esportare il PDF.')
    return
  }

  win.document.write(`<!DOCTYPE html>
<html lang="it">
<head>
  <meta charset="UTF-8">
  <title>${title}</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body {
      font-family: Georgia, 'Times New Roman', serif;
      max-width: 800px;
      margin: 2cm auto;
      color: #1a1a2e;
      font-size: 11pt;
      line-height: 1.65;
    }
    .doc-title {
      font-size: 18pt;
      font-weight: 700;
      color: #534AB7;
      border-bottom: 2px solid #534AB7;
      padding-bottom: 8px;
      margin-bottom: 24px;
    }
    h2 {
      font-size: 13pt;
      font-weight: 700;
      color: #1a1a2e;
      border-bottom: 1px solid #EEEDFE;
      padding-bottom: 4px;
      margin-top: 28px;
      margin-bottom: 10px;
    }
    h3 {
      font-size: 11.5pt;
      font-weight: 600;
      color: #534AB7;
      margin-top: 18px;
      margin-bottom: 6px;
    }
    p { margin-bottom: 10px; }
    ul, ol { margin-left: 22px; margin-bottom: 12px; }
    li { margin-bottom: 4px; }
    strong { font-weight: 700; }
    em { font-style: italic; color: #4B5563; }
    blockquote {
      border-left: 4px solid #534AB7;
      padding: 8px 16px;
      background: #F8F7FF;
      margin: 14px 0;
      font-style: italic;
      color: #4B5563;
      border-radius: 0 6px 6px 0;
    }
    .footer {
      margin-top: 48px;
      padding-top: 10px;
      border-top: 1px solid #ddd;
      font-size: 9pt;
      color: #999;
      text-align: right;
      font-family: Arial, sans-serif;
    }
    @media print {
      body { margin: 1.5cm; }
      h2 { page-break-after: avoid; }
    }
  </style>
</head>
<body>
  <p class="doc-title">${title}</p>
  ${content}
  <p class="footer">Master of Courses — generato il ${new Date().toLocaleDateString('it-IT')}</p>
</body>
</html>`)

  win.document.close()
  win.focus()
  setTimeout(() => {
    win.print()
    win.close()
  }, 300)
}

// ─── Utility: export Markdown ─────────────────────────────────────────────────

function htmlToMarkdown(html: string): string {
  return html
    // Headings
    .replace(/<h2[^>]*>([\s\S]*?)<\/h2>/gi, (_, t) => `\n## ${t.replace(/<[^>]+>/g, '').trim()}\n`)
    .replace(/<h3[^>]*>([\s\S]*?)<\/h3>/gi, (_, t) => `\n### ${t.replace(/<[^>]+>/g, '').trim()}\n`)
    // Inline formatting
    .replace(/<strong>([\s\S]*?)<\/strong>/gi, '**$1**')
    .replace(/<b>([\s\S]*?)<\/b>/gi, '**$1**')
    .replace(/<em>([\s\S]*?)<\/em>/gi, '_$1_')
    .replace(/<i>([\s\S]*?)<\/i>/gi, '_$1_')
    // Blockquote
    .replace(/<blockquote[^>]*>([\s\S]*?)<\/blockquote>/gi, (_, t) =>
      t
        .replace(/<[^>]+>/g, '')
        .trim()
        .split('\n')
        .map((l: string) => `> ${l.trim()}`)
        .filter((l: string) => l.trim() !== '>')
        .join('\n') + '\n\n'
    )
    // List items
    .replace(/<li[^>]*>([\s\S]*?)<\/li>/gi, (_, t) => `- ${t.replace(/<[^>]+>/g, '').trim()}\n`)
    .replace(/<[ou]l[^>]*>/gi, '\n')
    .replace(/<\/[ou]l>/gi, '\n')
    // Paragraphs
    .replace(/<p[^>]*>([\s\S]*?)<\/p>/gi, (_, t) => `${t.replace(/<[^>]+>/g, '').trim()}\n\n`)
    // Strip remaining tags
    .replace(/<[^>]+>/g, '')
    // Clean up whitespace
    .replace(/\n{3,}/g, '\n\n')
    .trim()
}

function exportMarkdown(title: string, content: string) {
  const md = `# ${title}\n\n${htmlToMarkdown(content)}`
  const blob = new Blob([md], { type: 'text/markdown;charset=utf-8' })
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url
  a.download = `${title.replace(/[^\w\s-]/g, '').replace(/\s+/g, '_').toLowerCase()}.md`
  document.body.appendChild(a)
  a.click()
  document.body.removeChild(a)
  URL.revokeObjectURL(url)
}

// ─── Componente principale ────────────────────────────────────────────────────

interface ActivityCardProps {
  activity: Activity
  onDelete: (id: string) => void
  isDeleting: boolean
}

export function ActivityCard({ activity, onDelete, isDeleting }: ActivityCardProps) {
  const [activeSection, setActiveSection] = useState<string | null>(null)
  const contentRef = useRef<HTMLDivElement>(null)

  const { processedHtml, sections } = parseAndInjectIds(activity.content ?? '')
  const showTOC = sections.length >= 2

  // Scroll tracking con IntersectionObserver
  useEffect(() => {
    if (!showTOC || !contentRef.current) return

    const headings = Array.from(contentRef.current.querySelectorAll('h2[id]'))
    if (headings.length === 0) return

    // Imposta la prima sezione come attiva di default
    setActiveSection(headings[0].id)

    const observer = new IntersectionObserver(
      (entries) => {
        // Trova il primo heading visibile
        const visible = entries
          .filter((e) => e.isIntersecting)
          .sort((a, b) => a.boundingClientRect.top - b.boundingClientRect.top)

        if (visible.length > 0) {
          setActiveSection(visible[0].target.id)
        }
      },
      {
        rootMargin: '-5% 0px -75% 0px',
        threshold: 0,
      }
    )

    headings.forEach((h) => observer.observe(h))
    return () => observer.disconnect()
  }, [showTOC, processedHtml])

  function scrollToSection(id: string) {
    const el = document.getElementById(id)
    if (el) {
      el.scrollIntoView({ behavior: 'smooth', block: 'start' })
    }
  }

  return (
    <div className="border border-gray-200 rounded-xl overflow-hidden">
      {/* ── Header ── */}
      <div className="flex items-center justify-between px-5 py-3 bg-gray-50 border-b border-gray-200">
        {/* Badge */}
        <div className="flex items-center gap-2 flex-wrap min-w-0">
          <h3 className="text-sm font-semibold text-[#1A1A2E] truncate">{activity.title}</h3>
          <span className="text-xs px-2 py-0.5 bg-[#EEEDFE] text-[#534AB7] rounded-full font-medium shrink-0">
            {ACTIVITY_LABELS[activity.type] ?? activity.type}
          </span>
          {activity.aiGenerated && (
            <span className="flex items-center gap-1 text-xs px-2 py-0.5 bg-purple-50 text-purple-600 border border-purple-100 rounded-full font-medium shrink-0">
              <Bot size={10} />
              Generato con IA
            </span>
          )}
        </div>

        {/* Pulsanti azione */}
        <div className="flex items-center gap-0.5 ml-3 shrink-0">
          {activity.content && (
            <>
              <button
                onClick={() => exportPDF(activity.title, activity.content!)}
                className="flex items-center gap-1 px-2.5 py-1.5 text-xs text-gray-500 hover:text-[#534AB7] hover:bg-[#EEEDFE] rounded-lg transition-colors"
                title="Esporta come PDF"
              >
                <FileDown size={13} />
                PDF
              </button>
              <button
                onClick={() => exportMarkdown(activity.title, activity.content!)}
                className="flex items-center gap-1 px-2.5 py-1.5 text-xs text-gray-500 hover:text-[#534AB7] hover:bg-[#EEEDFE] rounded-lg transition-colors"
                title="Esporta come Markdown"
              >
                <FileText size={13} />
                MD
              </button>
            </>
          )}
          <button
            onClick={() => onDelete(activity.id)}
            disabled={isDeleting}
            className="p-1.5 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors disabled:opacity-40 ml-1"
            title="Elimina attività"
          >
            <Trash2 size={14} />
          </button>
        </div>
      </div>

      {/* ── Body: TOC + Contenuto ── */}
      {activity.content && (
        <div className="flex items-start">
          {/* TOC laterale (solo se ci sono ≥ 2 sezioni h2) */}
          {showTOC && (
            <div className="w-44 shrink-0 border-r border-gray-100 bg-[#FAFAFA] sticky top-4 self-start">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide px-4 pt-4 pb-2">
                Sezioni
              </p>
              <nav className="px-2 pb-4 space-y-0.5">
                {sections.map(({ id, label }) => (
                  <button
                    key={id}
                    onClick={() => scrollToSection(id)}
                    title={label}
                    className={`w-full text-left text-xs px-3 py-2 rounded-lg transition-colors leading-snug ${
                      activeSection === id
                        ? 'bg-[#EEEDFE] text-[#534AB7] font-semibold'
                        : 'text-gray-500 hover:bg-gray-100 hover:text-gray-700'
                    }`}
                  >
                    {/* Tronca label troppo lunga */}
                    {label.length > 40 ? label.slice(0, 38) + '…' : label}
                  </button>
                ))}
              </nav>
            </div>
          )}

          {/* Contenuto HTML renderizzato */}
          <div
            ref={contentRef}
            className="ai-content px-6 py-5 flex-1 min-w-0"
            dangerouslySetInnerHTML={{ __html: processedHtml }}
          />
        </div>
      )}
    </div>
  )
}
